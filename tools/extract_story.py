...
from game.audio.music_map import LOCATION_MUSIC, SCREEN_MUSIC  # noqa: E402
...
def _check_music(locations):
    ...
    if unmapped:
        problems.append(f"locations with no entry in game/audio/music_map.py: {', '.join(unmapped)}")
    stale = sorted(k for k in LOCATION_MUSIC if k not in locations)
    if stale:
        problems.append(f"game/audio/music_map.py names locations that no longer exist: {', '.join(stale)}")
    ...
    desktop = {p.stem for p in (ROOT / "assets" / "audio" / "music").glob("*.wav")}
    no_wav = sorted(named - desktop)
    if no_wav:
        problems.append(f"tracks with no .wav for the desktop game in assets/audio/music: "
                        f"{', '.join(no_wav)}")
...
def _write_music_map(data):
    ...
    lines = [
        ...
        "",
        "Generated by `tools/extract_story.py` from `game/audio/music_map.py`. Edit",
        "`music_map.py`, not this file. The desktop game reads the same table. The build fails if any location is",
...
def main():
    ...
    for region_id, region in world.regions.items():
        for loc_id, loc in (region.get("locations") or {}).items():
            ...
            # Music comes only from game/audio/music_map.py; see _check_music().
